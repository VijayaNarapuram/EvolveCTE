...
function LoadStudentBanner() {
    if ($("#studentBanner").length > 0) {
        AccountId = window.localStorage.getItem('portalAccountId');

        var htmlString = "";
        htmlString = '<div class="row"><div class="col-sm-8" id="PortalLinkedStudentsList">';
        //var  FirstRowString = '<div class="form-group col-sm-12" ';
        var FirstRowString = '<div class="form-group col-sm-3">';
        FirstRowString += '<label for="">Students Associated:</label>';
        FirstRowString += '</div>';
        FirstRowString += '<div class="form-group col-sm-3">';
        FirstRowString += '<select class="form-control" id="ddlStudentBannerLinkedStudents" onchange=Dropdownchange();></select>';
        //FirstRowString += '</div>';
        ...
        var value;
        if (AccountId > 0) {
            $.getJSON('/PortalUser/GetPortalLinkedStudents/' + AccountId,
        function (optionsData) {
            var select = $("#ddlStudentBannerLinkedStudents");
            select.empty();
            ...
            //$('#ddlStudentBannerLinkedStudents').val(window.localStorage.getItem('LinkedStudentsVal'));
            $('#ddlStudentBannerLinkedStudents').val(window.localStorage.getItem('StudentID'));
            var selectedStudentDetails = $('#ddlStudentBannerLinkedStudents option:selected').val();
            var selectedStudentID = selectedStudentDetails;
            //window.localStorage.removeItem("StudentID");
            //window.localStorage.removeItem('StudentName');

            //window.localStorage.setItem("StudentID", selectedStudentID);
            //window.localStorage.setItem('StudentName', $("#ddlStudentBannerLinkedStudents option:selected").text());

            //$("#lblSelectedStudent").text('');
            //$("#lblSelectedStudent").text($("#ddlStudentBannerLinkedStudents option:selected").text());
            //value = (studentId != undefined && studentId != null && studentId != 0 && studentId != '') ? studentId : selectedStudentID;
        });
        }
    }
}
...
function Dropdownchange() {
    var selectedStudentDetails = $('#ddlStudentBannerLinkedStudents').val();
    var selectedStudentID = (selectedStudentDetails != null) ? selectedStudentDetails : null;
    window.localStorage.removeItem('StudentID');
    window.localStorage.removeItem('StudentName');
    window.localStorage.removeItem('PersonId');
    //window.localStorage.removeItem('LinkedStudentsVal');
    window.localStorage.setItem("StudentID", selectedStudentID);
    window.localStorage.setItem('StudentName', $("#ddlStudentBannerLinkedStudents option:selected").text());
   // window.localStorage.setItem('LinkedStudentsVal', $("#ddlStudentBannerLinkedStudents option:selected").val());
    if (selectedStudentID != 'undefined' && selectedStudentID > 0) {
        /// GET STUDENT Pattern Details /
        var url1 = "/PortalUser/GetSelectedStudentDetails/";
        var modelobj1 = {
            StudentId: selectedStudentID,
            portalAccountId: AccountId
        };
        //Catch the result sent from Post Back call
        var response1 = SendRequestJSON(url1, modelobj1);
        window.localStorage.setItem('SchoolCalendarId', response1.strSchoolYearPL);
        window.localStorage.setItem('StudentSchoolId', response1.strSchoolId);
        window.localStorage.setItem('PersonId', response1.strPersonId);
        window.localStorage.setItem('StudentSchoolName', response1.strSchoolName);
